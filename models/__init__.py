...
def get_net(input_depth, NET_TYPE, pad, upsample_mode, n_channels=3, act_fun='LeakyReLU', skip_n33d=128, skip_n33u=128,
    ...
    if NET_TYPE == 'ResNet':
        ...
        # UNet_new requires numeric convolution padding. The generic `pad`
        # argument may be a skip-network mode such as 'reflection', so it is
        # intentionally not forwarded here.
        net = UNet_new(num_input_channels=input_depth, num_output_channels=3,
                       feature_scale=4, more_layers=0, concat_x=False,
                       upsample_mode=upsample_mode, pad=1,
                       norm_layer=nn.InstanceNorm2d, need_sigmoid=True,
                       need_bias=True)
