...
class UNet_new(nn.Module):
    def __init__(self, num_input_channels=3, num_output_channels=3,
                 ...
                 upsample_mode='deconv', pad=1, norm_layer=nn.InstanceNorm2d, need_sigmoid=True, need_bias=True):
        ...
        self.final = conv(filters[0], num_output_channels, 1, bias=need_bias, pad=0)
...
class unetConv2(nn.Module):
    def __init__(self, in_size, out_size, norm_layer, need_bias, pad=1):
        ...
        if norm_layer is not None:
            self.conv1 = nn.Sequential(nn.Conv2d(in_size, out_size, kernel_size=3, padding=pad, bias=need_bias),
                                       norm_layer(out_size),
                                       nn.ReLU(), )
            self.conv2 = nn.Sequential(nn.Conv2d(out_size, out_size, kernel_size=3, padding=pad, bias=need_bias),
                                       norm_layer(out_size),
                                       nn.ReLU(), )
        else:
            self.conv1 = nn.Sequential(nn.Conv2d(in_size, out_size, kernel_size=3, padding=pad, bias=need_bias),
...
class unetUp(nn.Module):
    def __init__(self, out_size, upsample_mode, need_bias, pad=1, same_num_filt=False):
        ...
        if upsample_mode == 'deconv':
            ...
            self.up = nn.Sequential(nn.Upsample(scale_factor=2, mode=upsample_mode),
                                    nn.Conv2d(num_filt, out_size, kernel_size=3, padding=1, bias=need_bias))
            self.conv = unetConv2(out_size * 2, out_size, None, need_bias, pad)
        else:
            assert False
...
def conv(in_channels, out_channels, kernel_size, bias=True, pad=0):
    return nn.Conv2d(in_channels, out_channels, kernel_size, padding=pad, bias=bias)
