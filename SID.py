"""SID-UNN with joint optimization of the network and WLS parameters.
...
def main():
    ...
    for iteration in range(args.iterations):
        ...
        if iteration % show_every == 0 or iteration == args.iterations - 1:
            print(
                "Iteration {:05d}  Loss {:.6f}  PSNR {:.3f}  "
                "alpha {:.6f} (grad {:.3e})  lambda {:.6f} (grad {:.3e})".format(
                    iteration,
                    total_loss.item(),
                    reconstruction_psnr,
                    alpha.item(),
            ...
            if plot and not args.no_plot:
                plot_image_grid(
                    [
                        ...
                        np.clip(torch_to_np(output_ema), 0, 1),
